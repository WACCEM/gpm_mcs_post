"""
Calculate mean ERA5 value from a 2D variable centered at each MCS and 
saves the output matching MCS tracks to a netCDF file.
"""
__author__ = "Zhe.Feng@pnnl.gov"
__date__ = "06-Aug-2021"
...
if __name__ == "__main__":
    ...
    mcs_alldates = pd.date_range(start=min_basetime.dt.strftime("%Y-%m").item(), end=max_basetime.dt.strftime("%Y-%m").item(), freq='MS')
    # Convert all MCS times and ERA5 times to date strings
    rmcs_dates = rmcs_basetime.dt.strftime('%Y%m')
    ...
    fillval = np.NaN
    VAR_avg = np.full((ntracks, ntimes), fillval, dtype=float)
    VAR_max = np.full((ntracks, ntimes), fillval, dtype=float)
    VAR_min = np.full((ntracks, ntimes), fillval, dtype=float)
    ...
    varlist = {
        ...
        f"{varname}_avg": (['tracks', 'times'], VAR_avg, VAR_attrs),
        f"{varname}_max": (['tracks', 'times'], VAR_max, VAR_attrs),
        f"{varname}_min": (['tracks', 'times'], VAR_min, VAR_attrs),
    }
    coordlist = {
        'tracks': (['tracks'], dsm['tracks'].data, dsm['tracks'].attrs),
        'times': (['times'], dsm['times'].data, dsm['times'].attrs),
    }
    gattrlist = {
