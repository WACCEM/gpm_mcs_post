...
if __name__ == "__main__":
    ...
    # Get the number of digits for 0 padding
    digits = len(str(nparts))

    for ipart in range(0, nparts): 
        part_name = str(ipart).zfill(digits)
        outfilename = f"{output_dir}mcs_era5_{varname}_{track_period}_p{part_name}.nc"
        # Select tracks for this part        
        coord_tracks = dsm['tracks'].sel(tracks=slice(track_start[ipart], track_end[ipart]))
        rmcs_lat = dsm['meanlat'].sel(tracks=slice(track_start[ipart], track_end[ipart]))
        rmcs_lon = dsm['meanlon'].sel(tracks=slice(track_start[ipart], track_end[ipart]))
        rmcs_basetime = dsm['base_time'].sel(tracks=slice(track_start[ipart], track_end[ipart]))
