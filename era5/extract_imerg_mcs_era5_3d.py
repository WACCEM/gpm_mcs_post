"""
Extracts ERA5 vertical profiles within a box area centered at each MCS and 
saves the output matching MCS tracks to a netCDF file.
"""
__author__ = "Zhe.Feng@pnnl.gov"
__date__ = "10-Sep-2021"
...
def main():
    ...
    outfilename = f"{output_dir}era5_{varname}_{track_period}.nc"
    ...
    if run_parallel == 0:
        ...
        for ifile in range(nfiles):        
            ...
            # Call function to calculate statistics
            result = dask.delayed(extract_era5_prof)(filename, mcs_time, mcs_lat, mcs_lon, level_lims, ny, nx, varname)
            results.append(result)
    ...
    fillval = np.NaN
    VAR_out = np.full((ntracks, ntimes, nlevels, 2*ny+1, 2*nx+1), fillval, dtype=float)
    xcoords = np.arange(0, 2*nx+1)
    ycoords = np.arange(0, 2*ny+1)
    xcoords_attrs = {'long_name':'longitude grids', 'units':'0.25deg'}
    ycoords_attrs = {'long_name':'latitude grids', 'units':'0.25deg'}
    ...
    varlist = {
        ...
        f"{varname}": (['tracks', 'times', 'level', 'y', 'x'], VAR_out, VAR_attrs),
    }
    coordlist = {
    ...
    gattrlist = {
        'Title': 'ERA5 extracted columns following MCS',
        'lon_box_size': nx*2+1,
        'lat_box_size': ny*2+1,
