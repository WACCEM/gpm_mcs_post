...
def merge_tiles(nx_g, ny_g, var_list, GlatB, GlatT, GlonL, GlonR, latB, latT, lonL, lonR):
    ...
    var_merge = np.full((4, ny_g, nx_g), np.nan, dtype=float)
...
def write_netcdf(out_filename,
    ...
    dsmap = xr.Dataset({'mcs_number': (['season', 'lat', 'lon'], mcs_number), \
                        'mcs_freq_ccs': (['season', 'lat', 'lon'], mcs_freq_ccs), \
                        'mcs_freq_pf': (['season', 'lat', 'lon'], mcs_freq_pf), \
                        'mcs_initiation_ccs': (['season', 'lat', 'lon'], mcs_initiation_ccs), \
                        'mcs_pfdiameter': (['season', 'lat', 'lon'], mcs_pfdiameter), \
                        'mcs_lifetime': (['season', 'lat', 'lon'], mcs_lifetime), \
                        'mcs_totalrain': (['season', 'lat', 'lon'], mcs_totalrain), \
                        'mcs_totalrainheavy': (['season', 'lat', 'lon'], mcs_totalrainheavy), \
                        'mcs_rainrateheavy': (['season', 'lat', 'lon'], mcs_rainrateheavy), \
                        'mcs_rainratemax': (['season', 'lat', 'lon'], mcs_rainratemax), \
                        'mcs_speed': (['season', 'lat', 'lon'], mcs_speed), \
                        'mcs_uspeed': (['season', 'lat', 'lon'], mcs_uspeed), \
                        'mcs_vspeed': (['season', 'lat', 'lon'], mcs_vspeed), \
                        'precipitation': (['season', 'lat', 'lon'], precipitation), \
                        'mcs_precipitation': (['season', 'lat', 'lon'], mcs_precipitation), \
                        'mcs_precipitation_frac': (['season', 'lat', 'lon'], mcs_precipitation_frac), \
                        'mcs_precipitation_freq': (['season', 'lat', 'lon'], mcs_precipitation_freq), \
                        'mcs_precipitation_intensity': (['season', 'lat', 'lon'], mcs_precipitation_intensity), \
                        }, \
                        coords={'season': (['season'], seasons), \
                                'lat': (['lat'], lat_G), \
                                'lon': (['lon'], lon_G)}, \
                        attrs={'title': 'MCS seasonal mean climatology', \
...
def main():
    ...
    out_filename = f'{outdir}mcs_seasonal_mean_map_{period}.nc'
    os.makedirs(outdir, exist_ok=True)

    # Input file names
    sfile_asia = f'{dir_asia}mcs_statsmap_seasonal_mean_{period}.nc'
    sfile_nam = f'{dir_nam}mcs_statsmap_seasonal_mean_{period}.nc'
    sfile_spac = f'{dir_spac}mcs_statsmap_seasonal_mean_{period}.nc'
    pfile_asia = f'{dir_asia}mcs_rainmap_seasonal_mean_{period}.nc'
    pfile_nam = f'{dir_nam}mcs_rainmap_seasonal_mean_{period}.nc'
    pfile_spac = f'{dir_spac}mcs_rainmap_seasonal_mean_{period}.nc'
